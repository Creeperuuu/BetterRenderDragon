...
public:
  Image()
      : mImageFormat(ImageFormat::Unknown), mWidth(0), mHeight(0),
        mUsage(ImageUsage::Unknown) {}
  Image(ImageFormat format, uint32_t width, uint32_t height, ImageUsage usage,
        Blob &&data)
      : mImageFormat(format), mWidth(width), mHeight(height), mUsage(usage),
  ...
  mce::Image::Image(const mce::Image &rhs)
      : mImageFormat(rhs.mImageFormat), mWidth(rhs.mWidth),
        mHeight(rhs.mHeight), mUsage(rhs.mUsage), mDepth(rhs.mDepth),
        mImageBytes(rhs.mImageBytes.clone()) {}

  inline Image clone() const {
    return {this->mImageFormat, this->mWidth, this->mHeight, this->mUsage,
            this->mImageBytes.clone()};
  }
  inline void copyRawImage(Blob const &blob) {
    this->mImageBytes = blob.clone();
  }
  inline bool isEmpty() const { return this->mImageBytes.empty(); }
  inline void resizeImageBytesToFitImageDescription() {
    this->mImageBytes =
        Blob{this->mWidth * this->mHeight * (int)this->mImageFormat};
  }
  inline void setImageDescription(uint32_t width, uint32_t height,
  ...
  inline void setRawImage(Blob &&buffer) {
    this->mImageBytes = std::move(buffer);
  }
};
